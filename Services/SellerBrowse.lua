...
ns.OnMessage("S", function(a, b, c, _, _, _, sender)
    ...
    C_Timer.After(math.random() * jitter, function()
        ...
        ns.EnqueueWhisper(("C~%s~%d~%s~%s~%s~%s"):format(sid, n, loc, flag, ns.MyGuild() or "", ns.MyValidFlag()), sender)
    end)
end)

-- C~sid~count~loc~hasNote~guild~valid: a seller's summary in reply to our scan (hasNote, guild
-- and the valid flag appended in that order).
ns.OnMessage("C", function(a, b, c, d, e, f, sender)

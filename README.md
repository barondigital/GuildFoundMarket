...
The real Auction House can't restrict who you buy from or sell to. Guild Found Craigslist
...
```
GFCc:MyCommunity:somesharedsecret
GFCp:My Main Guild:MG
GFCp:My Second Guild:MG2
```

- `GFCc:<name>:<secret>` — the shared secret. Only members who can read this (i.e. members of
  the listed guilds) can join the marketplace. Pick any secret string.
- `GFCp:<exact guild name>:<short tag>` — one per guild in your confederation.
...
- `/gfc` (or `/craigslist`) — open the window. Also available via the minimap button.
...
2. Make sure the folder is named `GuildFoundCraigslist`.

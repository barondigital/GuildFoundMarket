    ...
    for i = 1, ROWS do
        ...
        for _, b in ipairs({ r.x, r.edit, r.codSend }) do
            b:SetScript("OnEnter", function(self)
                ...
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT"); GameTooltip:SetText(self.tip, 1, 1, 1, 1, true); GameTooltip:Show()
            end)
            b:SetScript("OnLeave", GameTooltip_Hide)
        end
...
local function buildStatusVersion()
    ...
    verBtn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText(ns.updateAvailable and "Click to see what's new in the update" or "Click to view the changelog", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
local function buildBuyerWidgets()
    ...
    findRefreshBtn:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:SetText("Ask the whole confederation again what they want to buy.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    ...
    local function renderText(s)
        ...
        if s.default and s.default ~= "" then
            ...
            reset:SetScript("OnEnter", function(self)
                GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                GameTooltip:SetText("Restore the example message, with every placeholder filled in.", 1, 1, 1, 1, true)
                GameTooltip:Show()
            end)
